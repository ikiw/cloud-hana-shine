...
function getBuinessPartners() {
	...
	var query = "SELECT \"PARTNERID\" FROM \"_SYS_BIC\".\"{{PACKAGE_NAME}}.data::EPM.MasterData.BusinessPartner\"";
...
function resetTableSequence(object){
	...
	switch (object) {
	case "addressSeqId":
		selectQuery = 'SELECT to_int(MAX("ADDRESSID") + 1) FROM "_SYS_BIC"."{{PACKAGE_NAME}}.data::EPM.MasterData.Addresses"';
		altQuery = 'ALTER SEQUENCE "_SYS_BIC"."{{PACKAGE_NAME}}.data::addressSeqId" RESTART WITH ';
		break;
	case "employeeSeqId":
		selectQuery = 'SELECT to_int(MAX("EMPLOYEEID") + 1) FROM "_SYS_BIC"."{{PACKAGE_NAME}}.data::EPM.MasterData.Employees"';
		altQuery = 'ALTER SEQUENCE "_SYS_BIC"."{{PACKAGE_NAME}}.data::employeeSeqId" RESTART WITH ';
		break;
	case "partnerSeqId":
		selectQuery = 'SELECT to_int(MAX("PARTNERID") + 1) FROM "_SYS_BIC"."{{PACKAGE_NAME}}.data::EPM.MasterData.BusinessPartner"';
		altQuery = 'ALTER SEQUENCE "_SYS_BIC"."{{PACKAGE_NAME}}.data::partnerSeqId" RESTART WITH ';
		break;
	case "purchaseOrderSeqId":
		selectQuery = 'SELECT to_int(MAX("PURCHASEORDERID") + 1) FROM "_SYS_BIC"."{{PACKAGE_NAME}}.data::EPM.Purchase.Header"';
		altQuery = 'ALTER SEQUENCE "_SYS_BIC"."{{PACKAGE_NAME}}.data::purchaseOrderSeqId" RESTART WITH ';
		break;
	case "salesOrderId":
		selectQuery = 'SELECT to_int(MAX("SALESORDERID") + 1) FROM "_SYS_BIC"."{{PACKAGE_NAME}}.data::EPM.Sales.Header"';
