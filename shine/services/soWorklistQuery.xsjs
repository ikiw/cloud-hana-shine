...
function getFilter() {
	...
	try {
		...
		query =  'SELECT TO_INT(SCORE()*100)/100 AS score, TO_NVARCHAR(COMPANYNAME) FROM "{{PACKAGE_NAME}}.data::EPM.MasterData.BusinessPartner" '
		...
		query = 'SELECT TO_INT(SCORE()*100)/100 AS score, TO_NVARCHAR(CITY) FROM "{{PACKAGE_NAME}}.models::AT_BUYER" '
