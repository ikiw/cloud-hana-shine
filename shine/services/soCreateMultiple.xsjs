...
var pstmt = conn.prepareStatement('SELECT "_SYS_BIC"."{{PACKAGE_NAME}}.data::salesOrderId".NEXTVAL as OverallId from Dummy');
...
lv_bp_id = overallsoData.PARTNERID;
// use prepared statement to avoid sql injection attacks
var pstmt = conn.prepareStatement('SELECT COMPANYNAME from "_SYS_BIC"."{{PACKAGE_NAME}}.data::EPM.MasterData.BusinessPartner" where PARTNERID = ?');
...
if (so_items) {
    ...
    for (i = 0; i < so_items.length; i++) {
		...
		pstmt = conn.prepareStatement('SELECT PRICE from "_SYS_BIC"."{{PACKAGE_NAME}}.data::EPM.MasterData.Products" where PRODUCTID = ?');
		...
		pstmt = conn.prepareStatement('insert into "_SYS_BIC"."{{PACKAGE_NAME}}.data::EPM.Sales.Item"'
		         + " values(?,?,?,'','EUR',?,?,?,'I','',?,'EA','')");
...
pstmt = conn.prepareStatement('insert into \"_SYS_BIC\".\"{{PACKAGE_NAME}}.data::EPM.Sales.Header\"'
